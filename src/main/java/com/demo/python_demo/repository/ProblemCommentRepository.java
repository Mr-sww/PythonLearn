...
public interface ProblemCommentRepository {
    ...
    @Insert("INSERT INTO problem_comment (problem_id, user_id, content, parent_id, likes, created_at, updated_at) VALUES (#{problemId}, #{userId}, #{content}, #{parentId}, 0, NOW(), NOW())")
    ...
    @Select("SELECT c.*, u.nickname, u.avatar FROM problem_comment c LEFT JOIN user u ON c.user_id = u.user_id WHERE c.problem_id = #{problemId} ORDER BY c.created_at DESC LIMIT #{offset}, #{pageSize}")
