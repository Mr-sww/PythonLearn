                                ...
                                <div v-for="(msg, idx) in chatMessages" :key="idx">
                                    <div v-if="msg.type === 'ai'" class="d-flex align-items-start">
                                        ...
                                        <div class="bg-light border border-primary/10 rounded-4 rounded-start-0 px-4 py-2" style="max-width:60%;min-width:80px;word-break:break-all;">
                                            <span class="text-dark" style="font-size:1rem;">{{ msg.content }}</span>
                                        </div>
                                    </div>
    ...
    methods: {
        ...
        sendMessage() {
            if (this.userMessage.trim()) {
                ...
                setTimeout(() => {
                    this.chatMessages.push({
                        type: 'ai',
                        content: this.getAIReply(this.userMessage),
                        timestamp: new Date()
                    });
                    this.scrollToBottom();
                }, 800);
        ...
        getAIReply(question) {
            ...
            if (question.includes('Python')) {
                return 'Python是一门简单易学、功能强大的编程语言，适合初学者和专业开发者。';
            }
            if (question.includes('Web')) {
                return '<b>Django</b>、<b>Flask</b>、<b>FastAPI</b>等都是Python流行的Web框架。';
            }
            return '这是一个模拟的AI回复。在实际应用中，这里会调用真实的AI API。';
        },
        scrollToBottom() {
            this.$nextTick(() => {
